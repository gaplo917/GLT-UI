...
/>`,
  ...
  <TableBody>
    ...
    },
    {
      ...
      name: 'Chart',
      description:
        "A themed wrapper around chart.js. Feed it labels + series for the common case — palette, fonts, grid, tooltip, and legend are all pulled from theme tokens and re-read automatically when the theme switches between light and dark. By default, pie/doughnut slices and single-series bars use a different palette color per category; multi-series and stacked charts use one color per series. On-chart annotations default on: category + value on circular charts, value only on bars/lines/points so axis ticks are not duplicated (dataLabels={false} to hide — use that when a custom label plugin already annotates). Long category names wrap onto multi-line ticks via categoryLabelMaxChars (breaks at commas, &, /, spaces); the max is clamped to the live panel width so mobile wraps instead of clipping, without shrinking tick font size. Colors accept palette tokens ('brand', 'success', …), CSS custom-property names, or raw CSS colors. Drop down to raw chart.js any time via the data, options, and plugins escape hatches. type=\"area\" is sugar for a filled line chart. wrapCategoryLabel() / resolveCategoryLabelMaxChars() are also exported.",
