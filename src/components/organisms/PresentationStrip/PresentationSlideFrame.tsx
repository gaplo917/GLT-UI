...
export function PresentationSlideFrame({
  ...
  return (
    ...
    >
      {showHeader ? (
        <header className="flex shrink-0 items-start justify-between gap-4 border-b border-[var(--border-color)] bg-[var(--bg-color)]/50 px-6 py-3">
          <div className="min-w-0">
            {kicker != null && (
              <p className="m-0 text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--brand-primary)]">
                {kicker}
              </p>
            )}
            {title != null && (
              <h2 className="m-0 mt-0.5 text-[20px] font-bold leading-tight tracking-tight text-[var(--strong-text-color)]">
                {title}
              </h2>
            )}
          </div>
          {brandMeta != null && (
            <div className="m-0 shrink-0 text-right text-[11px] font-medium uppercase tracking-wide text-[var(--secondary-text-color)]">
              {brandMeta}
            </div>
          )}
        </header>
      ...
      <div
        className={
          showHeader
            ? 'flex min-h-0 flex-1 flex-col gap-2.5 px-5 py-3'
